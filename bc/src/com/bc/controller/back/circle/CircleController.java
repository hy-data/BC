package com.bc.controller.back.circle;
...
public class CircleController extends BaseController {
	...
	public void delete(PrintWriter out){
		logBefore(logger, "删除Circle");
		PageData pd = new PageData();
		try{
			pd = this.getPageData();
			circleService.delete(pd);
			out.write("success");
			out.close();
		} catch(Exception e){
	...
	public ModelAndView goAdd(){
		logBefore(logger, "去新增Circle页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			mv.setViewName("back/circle/circle_edit");
	...
	public ModelAndView goEdit(){
		logBefore(logger, "去修改Circle页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			pd = circleService.findById(pd);	//根据ID读取
			mv.setViewName("back/circle/circle_edit");
	...
	public Object deleteAll() {
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				circleService.deleteAll(ArrayDATA_IDS);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
	...
	public ModelAndView exportExcel(){
		logBefore(logger, "导出Circle到excel");
		...
		try{
			...
			List<PageData> varOList = circleService.listAll(pd);
			List<PageData> varList = new ArrayList<PageData>();
			for(int i=0;i<varOList.size();i++){
				PageData vpd = new PageData();
				vpd.put("var1", varOList.get(i).get("SITEID").toString());	//1
