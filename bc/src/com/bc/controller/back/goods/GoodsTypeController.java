...
public class GoodsTypeController extends BaseController {
	...
	public void delete(PrintWriter out){
		logBefore(logger, "删除GoodsType");
		PageData pd = new PageData();
		try{
			pd = this.getPageData();
			goodstypeService.delete(pd);
			out.write("success");
			out.close();
		} catch(Exception e){
	...
	public ModelAndView goAdd(){
		logBefore(logger, "去新增GoodsType页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
			mv.setViewName("back/goods/type/goodstype_edit");
	...
	public ModelAndView goEdit(){
		logBefore(logger, "去修改GoodsType页面");
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		try {
	...
	public Object deleteAll() {
		logBefore(logger, "批量删除GoodsType");
		PageData pd = new PageData();		
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				goodstypeService.deleteAll(ArrayDATA_IDS);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
	...
	public ModelAndView exportExcel(){
		logBefore(logger, "导出GoodsType到excel");
		...
		try{
			...
			for(int i=0;i<varOList.size();i++){
				...
				vpd.put("var2", varOList.get(i).getString("GOODS_GROUP"));	//2
				vpd.put("var3", varOList.get(i).get("STATUS").toString());	//3
				vpd.put("var4", varOList.get(i).get("PROPERTY_NUM").toString());	//4
				varList.add(vpd);
			}
